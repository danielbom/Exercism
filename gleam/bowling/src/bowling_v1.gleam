...
fn game_is_complete(game: Game) -> Bool {
  case list.length(game.frames) {
    ...
    frames_count if frames_count == max_frames ->
      case game.frames {
        [Frame([_]), ..] -> False
        [Frame([snd, fst]), ..] if snd + fst == max_pins -> False
        _ -> True
      }
    frames_count if frames_count == max_frames + 1 ->
      case game.frames {
        [Frame([_]), Frame([_]), ..] -> False
        _ -> True
      }
    _ -> True
  }
  ...
  case game_is_complete(game) {
    ...
    False -> {
      case game.frames {
        ...
        [Frame([fst]), ..] if fst == max_pins -> {
          Ok(Game([Frame([knocked_pins]), ..game.frames]))
        }
        [Frame([fst]), ..] if fst + knocked_pins > max_pins -> {
...
fn score_loop(frames: List(Frame), count: Int, acc: Int) -> Int {
  case frames {
    ...
    [Frame([single]), ..next_frames] if single == max_pins -> {
      case next_frames {
        [Frame([snd, fst]), ..] -> {
        ...
        [Frame([fst]), Frame([_, snd]), ..] -> {
          score_loop(next_frames, count + 1, acc + single + fst + snd)
        }
        _ -> panic
      }
    }
    [Frame([snd, fst]), ..next_frames] if snd + fst == max_pins -> {
      case next_frames {
        [Frame([next]), ..] -> {
